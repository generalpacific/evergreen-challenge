...
def tfidf(data, isTrain):
  if isTrain == True:
    filename = 'cache/tfidf_train'
  else:
    filename = 'cache/tfidf_test'
  try:
...
def selectModel(classifier):
  if classifier == 0:
    ...
    model = RandomForestClassifier(n_estimators=10, max_features="auto", max_depth=None, bootstrap=True, n_jobs=4,
                                   criterion="entropy")
...
def egon(rawData, trainFile, testFile, classifier, boilerplateOnly):
  ...
  if boilerplateOnly == 0:
    ...
    X = tfidf(trainDataBoilerplate, True)
    ...
    topicsVector = getLdaTopicsVector(trainDataBoilerplate)
